...
class CustomUrlRule extends CBaseUrlRule {
    ...
    var $skey = "KEYUNICA12345612";
    var $pubkey = "KEYUNICA12345612";
    var $privkey = "KEYUNICA12345612";

    

    public function createUrl($manager, $route, $params, $ampersand) {
        $paramString = array();
        $paramStringRuta  = explode("/", $route);
        $cosas = '';
        ...
        $paramStringRutaEncoded=$this->encode($route) ;
        //print_r($paramStringRutaEncoded);
        $tmp=$this->encode($route. '/' . $paramStringEncoded);
        //print_r($tmp." deco: ".$this->decode($tmp)."<br>");
        return $this->encode($route. '/' . $paramStringEncoded);
    }

    public function parseUrl($manager, $request, $pathInfo, $rawPathInfo) {
           $pathInfo=$this->decode($pathInfo);
           // $pathInfo=$this->decode($pathInfo);
            $pathParams = explode("/", $pathInfo);
        //$pathParams[0]=$this->decode($pathParams[0]);
